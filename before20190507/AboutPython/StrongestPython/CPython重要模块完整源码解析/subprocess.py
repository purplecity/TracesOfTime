...
# 首先要了解 按照官网上说是替代os.sysatem os.spawn*的
# 所以这是替代c中的system() popen()的
# c中system popen是fork exec执行一个shell waitpid然后 return结果 父进程和子进程之间说是与管道进行通信
# 在python官方文档中也是建议 使用run 更高级的化直接使用popen方法
# run：Run the command described by args. Wait for command to complete, then return a CompletedProcess instance.
# popen： Execute a child program in a new process. On POSIX, the class uses os.execvp()-like behavior to execute the child program. 没有返回值 可以用with控制块
# 完事 不再阅读源码  
